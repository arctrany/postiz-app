...
> 以下平台已通过代码审计确认完整实现，无需改动：

| 平台 | 协议 | 备注 |
|------|------|------|
| X (Twitter) | OAuth 1.0a | twitter-api-v2 库 |
| LinkedIn | OAuth 2.0 | 支持图文、视频、PDF |
| YouTube | OAuth 2.0 | 共用 Google provider |
| TikTok | OAuth 2.0 | 国际版 |
| Facebook / Instagram | OAuth 2.0 | Meta Graph API |
| Pinterest | OAuth 2.0 | 已实现 |
| Discord | OAuth 2.0 | 已实现 |
| Reddit | OAuth 2.0 | 已实现 |
| Mastodon | OAuth 2.0 | 支持自定义实例 |
| Slack | OAuth 2.0 | 已实现 |
| Threads | Meta OAuth | Graph API |
| Bluesky | XRPC | AT Protocol |
| Dribbble | OAuth 2.0 | 已实现 |
| Beehiiv / Listmonk | API Key | 无需 OAuth |
